...
impl TeMap {
    ...
    pub fn from_file(file_name: &str, maps_path: String) -> Self {
        ...
        let json_temap = std::fs::read_to_string(maps_path + "/" + file_name + ".temap")
        ...
        if let json::JsonValue::Array(models) = json_temap {
            ...
            for dict in models {
                if let json::JsonValue::Object(model) = dict {
                    ...
                    let offsets: Vec<(f32, f32, f32)> = if let json::JsonValue::Array(off) =
                        model.get("offsets").expect("Invalid .temap")
                    {
                        off.iter()
                            .map(|o| {
                                if let json::JsonValue::Array(values) = o {
                                    (
    ...
    pub fn save(&self, file_name: &str, maps_path: String) {
        ...
        let file_name = file_name.trim_end_matches(".temap");
        std::fs::write(maps_path + "/" + file_name + ".temap", contents)
