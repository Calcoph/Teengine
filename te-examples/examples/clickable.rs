...
use te_renderer::{
    ...
    state::{GpuState, Section, TeColor, TeState, Text},
    text::FontReference,
};
...
pub(crate) const SQUARE_IND: &[u32] = &[1, 3, 2, 1, 2, 0];
...
pub(crate) fn main() {
    ...
    let (event_loop, te_gpu, te_window, te_state) =
        pollster::block_on(te_player::prepare(config, true)).expect("Couldn't initilize");
    let mut te_gpu = Rc::<_>::try_unwrap(te_gpu)
        .expect("Unreachable")
        .into_inner();
    let te_window = Rc::<_>::try_unwrap(te_window)
        .expect("Unreachable")
        .into_inner();
    let mut te_state = Rc::<_>::try_unwrap(te_state)
        .expect("Unreachable")
        .into_inner();

    let img = image::open("resources/default_texture.png")
        .expect("make sure default_texture.png exists in \"resources\" directory");
    let img = img
        .as_rgba8()
        .expect(r#""resources/default_texture.png" should have an alpha channel, but it doesn't"#);
    let square_model = Model::new_simple(
        SQUARE_VERT.into(),
        SQUARE_IND.into(),
        img,
        &te_gpu,
    ...
    */
    ...
    let font = te_state
        .load_font(String::from("CascadiaCode.ttf"), &te_gpu)
    ...
    let mut clickable_depth_texture =
        te_renderer::texture::Texture::create_depth_texture_surfaceless(
            &te_gpu.device,
            WIN_WIDTH,
            WIN_HEIGHT,
...
async fn render(
    ...
    *last_render_time = now;
    ...
    if see_clickable {
        ...
        texts.draw_text(|texts| te_state.render(&view, &te_gpu, &mut encoders, texts));
    }
    let clickable_encoder = &mut encoders[0];
    ...
    let destination_buffer = wgpu::ImageCopyBufferBase {
        buffer: &destination,
    ...
    slice.map_async(wgpu::MapMode::Read, move |v| {
        sender.send(v).expect("Couldn't send")
    });
